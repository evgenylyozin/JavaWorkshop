...
public class Exercise1 {
    public static void main(String[] args) throws InterruptedException{
        ...
        threadA.setName("A");
        ObjectIncrement _threadB = new ObjectIncrement(_threadA, 5,
                simulationSize);
        Thread threadB = new Thread(_threadB);
        threadB.setName("B");
        ObjectDecrement _threadC = new ObjectDecrement(_threadA, 10,
                simulationSize);
...
class ObjectIncrement implements Runnable {
    ...
    int objectFreq;
    int simulationSize;
    ObjectIncrement(ObjectCounter _threadToCount, int _objectFreq, int
            _simulationSize) {
...
class ObjectDecrement implements Runnable {
    ...
    int objectFreq;
    int simulationSize;
    ObjectDecrement (ObjectCounter _threadToCount, int _objectFreq, int
            _simulationSize) {
